...
Master plans decompose their work into numbered phases. No plan here
has needed separate phase files -- each is small enough that the
...
| 2026-09-03 | [Library API, missing verbs, and the shakenfist.k3s collection](library-api-and-collection.md) | Make the orchestration callable from Python, add the verbs a daemon needs, and ship an optional Ansible collection for cluster bringup | Not started | 1. Library API, 2. Client construction, 3. Missing verbs, 4. First release, 5. The collection, 6. Push audit |
