...
- **Return to phase 2's client construction once
  [shakenfist/client-python#404](https://github.com/shakenfist/client-python/issues/404)
  ships.** Phase 2 forces `ASYNC_CONTINUE` by assigning to
  `client.async_strategy` on the client `sf-client` built, because
  `apiclient` offers no per-call or copy-returning way to say "this
  operation must not block" (phase 2, decision 2). That issue proposes
  both shapes. When one lands, replace the assignment in
  `_bind_namespace_context()` with it and raise the
  `shakenfist_client >=` floor in `pyproject.toml`. This entry exists
  because the assignment will keep working indefinitely, so nothing
  will break to remind us.
