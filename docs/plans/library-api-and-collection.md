...
- **Resuming an interrupted `create`.** Phase 3 reads `md['state']`
  for the first time and scopes itself to detection and safe teardown
  (its decision 5): `create` distinguishes "already built" from
  "interrupted, delete it", `delete` works on a cluster that never
  reached `created`, and the health verb reports the state. Resuming
  a half-built cluster needs a resume point per phase of `create()`,
  against metadata a second writer may have touched, and validating it
  means killing a real create at each of those points. It is deferred
  rather than declined: if conductor needs it, it will need it against
  a state machine that is read at all, which is what phase 3 builds.
